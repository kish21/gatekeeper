...
> fail-safe webhook alerts on verify-failure/deny-spike). **181 tests green ×2 runs.** See `#Build log` rows
> 6–9. **Remaining in M3: M3.5** (connector-onboarding runbook + 3rd credentialed connector) + the two
> user-action proofs above, then the M3 `/eval` (re-measure the aspirational +<5 ms HTTP overhead).
