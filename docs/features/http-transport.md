...
| No unauthenticated enumeration | `tools/list` resolves the token fail-closed → an unauthenticated caller gets an **empty list** (empty-not-raise is load-bearing: the SDK's `call_tool` wrapper refreshes its tool cache through the list handler, and a raise there would short-circuit the ledgered deny). **`tools/call` closes the matching oracle (security-review hardening):** an unauthenticated caller gets the SAME generic `denied:` for a real tool and a nonexistent one — the helpful `unknown tool` answer is gated behind a valid token, so unknown-vs-known can't be probed. Real-tool denies are still ledgered (they flow through the pipeline); the unknown-name branch touches no governed resource. Both asserted by `test_http_unauthenticated_is_fail_closed_and_ledgered`. |
