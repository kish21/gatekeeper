...
> [docs/features/config-driven-any-server.md](../features/config-driven-any-server.md).
...
> *real-Entra proof = user action, guide in the feature doc*) · **M3.3 container ✅** (multi-stage
> non-root image, /data ledger volume, CI container job, Azure Container Apps guide; live-verified in
> the real local container; *actual Azure run = user action, copy-paste guide ready*) · **M3.4
> observability ✅** (/metrics Prometheus text + overhead-p95-vs-budget gauge, `gatekeeper stats`,
> fail-safe webhook alerts on verify-failure/deny-spike) · **M3.5 runbook ✅** (connector onboarding,
> config+`.env`-only, SLA template). **181 tests green ×2 runs.** See `#Build log` rows 6–10.
> **Remaining in M3 (all user-action proofs, zero code):** real-Entra token (M3.2 doc has the guide) ·
> actual Azure run (M3.3 guide is copy-paste) · one live credentialed connector run (M3.5 runbook,
> GitHub block paste-ready). Then the M3 `/eval` (re-measure the aspirational +<5 ms HTTP overhead).
...
| **HTTP transport (M3.1)** — MCP **Streamable HTTP** binding of the same pipeline: shared proxy-surface builder (`transport/surface.py`) refactored out of stdio so the two transports CANNOT drift; FastAPI+uvicorn **single-worker** app (`/mcp` exact-path route + `/healthz`); per-request `Authorization: Bearer` extracted by the transport, **resolved + recorded in the pipeline** (ADR-008, multi-principal on one gateway); `serve --transport stdio\|http` | ✅ **M3.1 exit met** · ADR-007 single-writer held by construction (no `workers` knob exists) · ADR-008: forged/missing bearer on `tools/call` → **ledgered `<unauthenticated>` DENY** then refused (never a silent 401); unauthenticated `tools/list` → **empty** (no enumeration; empty-not-raise is load-bearing — the SDK's call_tool wrapper refreshes its tool cache through the list handler, a raise there would skip the ledgered deny; found by the adversarial test) · `tools/call` closes the matching enumeration oracle (`/security-review` finding): unauthenticated callers get the SAME generic `denied:` for real and nonexistent tools, real-tool denies still ledgered · ADR-009: non-loopback bind **refuses boot** without `http_allow_non_loopback: true` ack (ack logs ADR-006 bearer-replay warning); unknown hostnames = non-loopback (fail-closed); SDK DNS-rebinding protection ON (rebound Host → 421) · config-only knobs (`http_path`/`http_allow_non_loopback`/`http_allowed_origins`), no new secret, token never logged | **Live (real `gatekeeper serve --transport http` ← real MCP client):** healthz 200 · 6 tools across both upstreams (incl. 3rd-party `time`) · alice read ALLOW transparent · bob write **DENY (Cedar default-deny)** · ledger holds prior **stdio** entries (seq 2–4) + these **HTTP** entries (seq 5–7) in ONE chain → `verify` OK 7 entries. **Tests: 150 (+19, 0 skipped)** — unit seams (exposure guard ×9, bearer parse ×8, knob flow, CLI fail-loud) + integration vs **real uvicorn + real MCP client + real subprocess upstream** (same-pipeline+verify-clean, forged-bearer ledgered deny, Host-header 421). ruff+format+mypy(strict) clean. | [docs/features/http-transport.md](../features/http-transport.md) |
| **OIDC identity adapter (M3.2)** — `OidcIdentityResolver` behind the **unchanged** `IdentityResolver` port: IdP JWT (per-request bearer from M3.1) validated — **JWKS signature, issuer, audience, expiry** — then config-driven **group→role map** → `Principal`; `adapters.identity: oidc` is a pure config swap (`static_token` stays dev default); stack = **PyJWT+PyJWKClient** (mini-ADR-010: gateway is a resource *server*; MSAL is an acquisition library — rejected; python-jose/authlib rejected) | ✅ fail-closed on EVERY path: expired/wrong-aud/wrong-iss/missing-exp/forged-signature/**HS-or-none downgrade**/unmapped-or-missing-groups/**JWKS outage**/unexpected exception → all `IdentityError` → pipeline ledgers the deny · **no default role ever** (authenticated ≠ authorized) · token never echoed in errors/logs/ledger (asserted) · boot fail-loud on half-config or failed discovery · no new secret (validates against the IdP's PUBLIC keys) | **Tests: 170 (+20)** — unit ×16 with **real RS256 JWTs** from a locally-generated RSA fake-IdP (only the JWKS *fetch* stubbed; full fail-closed matrix + factory dispatch) + integration ×2 over the **LIVE HTTP transport** (real uvicorn+MCP client: operator JWT allowed w/ JWT subject as ledger principal · readonly JWT Cedar-denied · expired JWT ledgered `<unauthenticated>` DENY · group change flips role with zero code · `verify`-clean). **Honest gap:** the *real Entra-issued* token clause needs the user's tenant — config-only setup guide written in the feature doc. ruff+mypy(strict) clean. | [docs/features/oidc-identity.md](../features/oidc-identity.md) |
| **Container + Azure-first deploy (M3.3)** — multi-stage `python:3.12-slim` image (wheel build stage → non-root runtime, uid 10001); entrypoint = `alembic upgrade head` (fail-loud) → `seed-demo` → `serve --transport http`; container config overlay (`deploy/container/platform.yaml`): 0.0.0.0 **with the explicit ADR-009 ack** (exposure+TLS = platform ingress), ledger on **/data** volume; new `transport.http_allowed_hosts` knob (hosted FQDN for the /mcp Host check); **CI `container` job** (build → boot w/ generated key → `/healthz` → in-container `verify`) on every push; deploy guide `docs/deploy/azure-container-apps.md` (ACR build, Container Apps, **1 replica pinned = ADR-007**, Azure Files /data, secretref HMAC key, probes, "make it real" step = OIDC + FQDN allowlist) | ✅ no secret in image/config (env-only; gitleaks-clean) · non-root · fail-loud migration · ADR-007 enforced operationally (min=max=1 replica documented + single-worker serve) · demo identities flagged smoke-only, guide mandates OIDC before real use · HEALTHCHECK = /healthz (stdlib, no curl layer) | **Live (real `docker build` + `docker run`, this session):** healthz 200 · MCP client over container HTTP: alice read **ALLOW transparent** (`Hello from GateKeeperAI…`), bob write **DENY** (Cedar default-deny) · `docker exec gatekeeper verify` → **OK 3 entries** (ledger on /data) · `/metrics` live · `stats` renders · CI job added so the image is re-proven every push. **Honest gap:** the *on-Azure* clause needs the user's subscription login — guide is copy-paste end-to-end; zero code remains. | [docs/features/container-deploy.md](../features/container-deploy.md) |
| **Observability surface (M3.4)** — operator health without a dashboard (deferred trigger intact): **`GET /metrics`** = Prometheus text exposition from a dependency-free in-process registry (`infra/metrics.py`) — calls by verdict, identity-denies, forward-errors, deny-rate, **overhead p95 gauge vs the configured `perf.overhead_p95_ms` budget** (p95 = same nearest-rank/exclude-forward method as the /eval bench, so live gauge ≡ bench); **`gatekeeper stats`** = ledger-derived history (calls counted once via decision entry, allow/deny rates, denies-by-principal, busiest tools); **alert hook** (`infra/alerts.py`): one JSON webhook (`GATEKEEPER_ALERT_WEBHOOK` in `.env`) fired on **verify-failure** (CLI) and **deny-spike** (config `observability.deny_spike`, one alert per breach episode, re-arms on drain), delivered **off the hot path** (worker thread, fire-and-forget) | ✅ alerting **fail-SAFE by contract** (a down receiver = one log line, never blocks/fails a governed call — governance stays fail-closed in the pipeline) · `/metrics` aggregates only (no principal/args/token — asserted) · webhook URL never logged (may embed a token) · no new dependency · config-driven knobs, no hardcoding | **Tests: 181 (+11)** — unit ×11 (p95 nearest-rank, no-fake-zero render, spike fires-once-and-re-arms, alerter disabled/payload/fail-safe, REAL `pipeline.handle` records allow/deny/identity-deny + overhead + exactly one spike alert, `stats` vs seeded real ledger + empty) + integration `/metrics` over live HTTP (counters+p95+budget present; principal/token absent) + **live in the running container** (`curl /metrics`, `docker exec gatekeeper stats`). ruff+mypy(strict) clean. | [docs/features/observability.md](../features/observability.md) |
| **Connector-onboarding runbook (M3.5)** — `docs/runbooks/connector-onboarding.md`: a fresh operator onboards a **credentialed** third-party MCP connector with **config + `.env` only** in <10 min — service-mapping template (incl. the explicit *annotate-every-destructive-tool* warning tied to the tracked classification→RBAC limitation) → `upstreams.yaml` block with `{from_env}` secrets → Cedar tightening example (`forbid … unless admin`) → live verification recipe (`tail`/`verify`/`stats`) → **SLA template** (owner, rotation, timeout, failure posture, audit review, alert) → troubleshooting table | ✅ serves the locked north-star secondary at enterprise grade · grounded ONLY in shipped mechanisms (secret-by-name injection, skip-on-bad-upstream, role RBAC, verify) — no invented features · no secret in the doc | Runbook content cross-checked against the live-subprocess tests that prove each mechanism (`test_upstream_secret_injection`, `test_any_server`, `test_proxy`) + this session's container run. **Honest gap:** the *live* credentialed (ServiceNow-class) run needs a real token = the operator's 5-min exercise, stated in the runbook itself; GitHub block is paste-ready. | [docs/runbooks/connector-onboarding.md](../runbooks/connector-onboarding.md) |
| **M3.3 first LIVE Azure run + hosted-deploy acceptance kit (2026-08-24)** — executed the previously docs-only Azure path end to end on a real subscription (West Europe), fixed every defect it surfaced, and added `scripts/probe_hosted.py` + `docs/runbooks/verify-hosted-deploy.md` (9 plain-language checks) so the path is re-verifiable | ⚠ **PARTIAL — governance proven, durable audit DISPROVEN.** ✅ live over public HTTPS · governed tool list · operator read ALLOW · read-only write **DENIED by Cedar, never forwarded** · unknown bearer DENIED fail-closed · `/metrics` live. ❌ `tail`/`verify` and restart-survival **fail on Azure Files (SMB)** — see #Known-issues | **Live (real deployment, probed from outside the cloud):** T1-T6 all PASS — T4 `denied by cedar policy: role 'readonly' may not write demo-files::write_file (default-deny)`; T5 `unknown or missing bearer token`. **Failed checks:** `audit.db` 0 bytes after served traffic, `Ledger table not found` from a 2nd process, records **lost** across a restart. **5 defects found+fixed** (CLI log-stream crash on cp1252; `share-rm` missing `-g`; `:latest` redeploys no-oping; documented `["<fqdn>:*"]` allow-list can never match a `:443` Host; rolling update = **two writers** on the single-writer ledger → `database is locked`). Probe keeps allowed/denied/**error** distinct so a transport failure cannot score as a passing deny (caught a false green in its own first run). ruff+ruff-format+mypy clean. | [docs/runbooks/verify-hosted-deploy.md](verify-hosted-deploy-first-run.md) |
| **Enterprise readiness — the six blockers from the strict-buyer review, plus M3.3's open defect (2026-09-08)** — durable Postgres ledger · verified approver identity (four-eyes, approver roles) · argument-aware policy · held-write notifications · export/retention/key-rotation · risk scoring | ✅ **M3.3 CLOSED.** The durable-audit clause that failed in August now passes: records survive the process that wrote them, a second process reads and verifies the chain, six concurrent writers produce ONE intact chain (240/240; with the advisory lock stubbed out the same run breaks it, so the serialization is load-bearing). Approvals name someone who PROVED they are that person, with the method recorded; a typed name is refused on any network-facing desk; nobody approves their own call. A `forbid` rule reading the arguments stops an admin. Retention removes records only under a signed checkpoint; a silent delete, an edited checkpoint and a forged one are all still caught. Rotation keeps every earlier record verifiable | **Live (real `gatekeeper serve --transport http` on a real PostgreSQL, real MCP client, real webhook receiver):** guardrail deny naming the rule · a write held and announced · a typed name **403**, the requester **403**, a reader **403**, the approver **200** · ledger reads `approved by priya via token` with the risk score · a second AND third process verify the chain · restarted the gateway and all 8 hosted-probe checks pass including **T8 SURVIVES RESTART**. **Tests: 342** (+88), ruff+format+mypy clean, CI now runs a real Postgres service | [durable-ledger](../features/durable-ledger.md) · [argument-aware-policy](../features/argument-aware-policy.md) · [ledger-operations](../features/ledger-operations.md) · [risk-scoring](../features/risk-scoring.md) |
...
today: UUID call_ids + single tenant) · ~~single-writer append assumption~~ **lifted 2026-09-08**: the
Postgres ledger serializes appends on a transaction-scoped advisory lock, so N replicas share one chain
(SQLite keeps the single-writer assumption, by construction) · **classification→RBAC evasion** — an
unannotated destructive tool whose name matches no `write_detection` pattern is classified read, so
`readonly` may call it (mitigation today: explicit `writes:` annotation; backstop: an LLM classifier per
ADR-005, whose seam is now open in `domain/risk`). All are tracked by asserted tests — see `#Tests`.

**Still open, stated plainly:** an archive file's integrity is your object store's problem once records
leave the ledger · retention is manual (cron, no scheduler) · `tenant` remains a column, not enforced
isolation · the risk scorer is deterministic arithmetic, and the LLM classifier of ADR-005 is not built ·
argument rules are string and attribute matching, not semantics: they raise the floor, the human hold and
the score are what catch what they miss.
...
| 2026-06-12 | **M3.1→M3.5 — Enterprise deployment readiness** (HTTP transport · OIDC identity · container+Azure guide · observability surface · connector-onboarding runbook) — built in one session by user instruction | **`/code-review` (high, 7 angles):** fixed test-harness duplication (shared `tests/integration/http_harness.py`) + recorded the container-overlay drift hazard. **`/security-review` (full):** OIDC+bearer traced clean (alg/aud/iss/kid/fail-closed); **fixed** an unauthenticated `tools/call` enumeration oracle + added an exposed-`static_token` boot warning; demo-token finding refuted (placeholders + fail-closed Host check + doc gating). **181 tests green ×4 local runs** + CI. | ✅ 4 feature docs + deploy guide + runbook + `#Architecture`/`#Build log`/marker/README/CHANGELOG — match code | `[Unreleased]` (HTTP/OIDC/observability all behind config; default posture unchanged = stdio + static_token) | **Additive + config-gated**; no migration. New transport/adapter only active when selected in `platform.yaml`. Rollback = **revert PR #28**. Signal: CI green incl. the new container job; `verify` clean over both transports. | [#28](https://github.com/kish21/gatekeeper/pull/28) |
